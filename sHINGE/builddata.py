...
# define the global variable FLAGS
        ...
        for _, line in enumerate(lines):
            ...
            for k in xx_dict:
                ...
                if k in roles:
                    ...
                    role_ind = next_role
                    next_role += 1
                    roles_indexes[k] = role_ind  # role2index dict {'soccer.football':1,...}
                    roles.add(k)                 # role set ('soccer','comput_football',...)
                if type(xx_dict[k]) == str:      # single value
                    val = xx_dict[k]
                    if val in values:
                        ...
                        val_ind = next_val
                        next_val += 1
                        values_indexes[val] = val_ind   # value2index dict  {'02ltg3':1,...}
                        values.add(val)                 # value set  ('02ltg3','023g42',...)
            ...
            if FLAGS.if_permutate == True:  # Permutate the elements in the fact for negative sampling or further computing the filtered metrics in the test process
                ...
                data[xx_dict['N']-2][aline] = [1]       # [{(roleid, valueid): [1]},...]
        ...
        for k in n_dict:
            ...
            if type(v) == str:
                ...
                if v_ind not in role_val[k_ind]:
                    role_val[k_ind].append(v_ind)   # all values related to one key
